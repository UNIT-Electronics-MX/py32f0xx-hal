...
fn main() -> ! {
    ...
    // PA2 as ADC analog input with pull-down (so floating reads ~0V)
    let mut adc_pin = gpioa.pa2.into_analog();
    
    // Configure PA2 with pull-down to avoid floating high readings
    unsafe {
        let gpioa_reg = &(*pac::GPIOA::ptr());
        gpioa_reg.pupdr.modify(|_, w| w.pupd2().pull_down()); // Pull-down on PA2
    }

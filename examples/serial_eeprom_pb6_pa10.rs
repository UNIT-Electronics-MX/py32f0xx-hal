...
fn main() -> ! {
    ...
    // Configure I2C pins: PB6=SCL, PA10=SDA with AF6
    ...
    unsafe {
        let gpioa_reg = &(*pac::GPIOA::ptr());
        let gpiob_reg = &(*pac::GPIOB::ptr());
        
        // PA10 (SDA): Open Drain + Pull-up
        gpioa_reg.otyper.modify(|_, w| w.ot10().set_bit());     // Open drain
        gpioa_reg.pupdr.modify(|_, w| w.pupd10().pull_up());    // Pull-up
        gpioa_reg.ospeedr.modify(|_, w| w.ospeed10().very_high_speed()); // High speed
        
        // PB6 (SCL): Open Drain + Pull-up  
        gpiob_reg.otyper.modify(|_, w| w.ot6().set_bit());     // Open drain
        gpiob_reg.pupdr.modify(|_, w| w.pupd6().pull_up());    // Pull-up
        gpiob_reg.ospeedr.modify(|_, w| w.ospeed6().very_high_speed()); // High speed
    }
    ...
    serial.write_str("=== USART2 + I2C FT24C32A DEMO - 9600 bps ===\r\n").ok();
    serial.write_str("PA0: TX (AF9) - USART2\r\n").ok();
    serial.write_str("PA1: RX (AF9) - USART2\r\n").ok();
    serial.write_str("PA10: SDA (AF6) - I2C FT24C32A\r\n").ok();
    serial.write_str("PB6: SCL (AF6) - I2C FT24C32A\r\n").ok();
    ...
    unsafe {
        let gpioa_reg = &(*pac::GPIOA::ptr());
        let gpiob_reg = &(*pac::GPIOB::ptr());
        let input_a = gpioa_reg.idr.read();
        let input_b = gpiob_reg.idr.read();
        
        if input_a.id10().bit_is_clear() {
            serial.write_str("ADVERTENCIA: PA10 (SDA) esta en LOW - posible corto a GND\r\n").ok();
        }
        if input_b.id6().bit_is_clear() {
            serial.write_str("ADVERTENCIA: PB6 (SCL) esta en LOW - posible corto a GND\r\n").ok();
        }
        
        if input_a.id10().bit_is_set() && input_b.id6().bit_is_set() {
    ...
    // EEPROM address (detectada en 0x51)
        ...
        match received {
            ...
            b't' => {
                ...
                unsafe {
                    ...
                    write!(serial, "PA10 (SDA): {}\r\n", 
                          if input_a.id10().bit_is_set() { "HIGH" } else { "LOW" }).ok();
                    write!(serial, "PB6 (SCL): {}\r\n", 
                          if input_b.id6().bit_is_set() { "HIGH" } else { "LOW" }).ok();

...
fn main() -> ! {
    ...
    // Configure I2C pins: PF1=SCL, PF0=SDA with AF12
    let _scl = gpiof.pf1.into_alternate_af12();
    let _sda = gpiof.pf0.into_alternate_af12();
    
    // Configure GPIO registers for I2C: Open Drain + Pull-up
    unsafe {
        let gpiof = &(*pac::GPIOF::ptr());
        
        // PF0 (SDA): Open Drain + Pull-up
        gpiof.otyper.modify(|_, w| w.ot0().set_bit());     // Open drain
        gpiof.pupdr.modify(|_, w| w.pupd0().pull_up());    // Pull-up
        gpiof.ospeedr.modify(|_, w| w.ospeed0().very_high_speed()); // High speed
        
        // PF1 (SCL): Open Drain + Pull-up  
        gpiof.otyper.modify(|_, w| w.ot1().set_bit());     // Open drain
        gpiof.pupdr.modify(|_, w| w.pupd1().pull_up());    // Pull-up
        gpiof.ospeedr.modify(|_, w| w.ospeed1().very_high_speed()); // High speed
    }
    ...
    serial.write_str("PF0: SDA (AF12) - I2C FT24C32A\r\n").ok();
    serial.write_str("PF1: SCL (AF12) - I2C FT24C32A\r\n").ok();
